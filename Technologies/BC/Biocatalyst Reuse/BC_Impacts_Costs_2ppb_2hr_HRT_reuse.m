...
for i=1:10
    for j=1:1000
Yield(h,i)=Production_Media_Required_L_DW_BO_2(h)/i;
Reuse(h,i,j)=Yield(h,i)/j;
Production_Media_Required_L_DW_m3(h,i,j)=Reuse(h,i,j);
%production media required (L) to treat m3 of water

%AMOUNTS%
%Amounts of Ingredients (kg) for Biocatalyst Production Media for 1m3 of DW
kg_sodium_phosphate_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_sodium_phosphate;
kg_soda_ash_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_soda_ash;
kg_protein_feed_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_protein_feed;
kg_acetic_acid_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_acetic_acid;
kg_sodium_perchlorate_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_sodium_perchlorate;
kg_water_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_water;
kg_sodium_chloride_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_sodium_chloride;
kg_ammonium_chloride_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_ammonium_chloride;
kg_potassium_chloride_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_potassium_chloride;
kg_magnesium_sulfate_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_magnesium_sulfate;
kg_calcium_chloride_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_calcium_chloride;
kg_EDTA_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_EDTA;
kg_potassium_hydroxide_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_potassium_hydroxide;
kg_iron_sulfate_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_iron_sulfate;
kg_cobalt_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_cobalt;
kg_zinc_monosulfate_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_zinc_monosulfate;
kg_copper_concentrate_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_copper_concentrate;
kg_nickel_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_nickel;
kg_selenium_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_selenium;
kg_molybdenum_m3_DW(h,i,j)=Production_Media_Required_L_DW_m3*BC_P_molybdenum;

%Total Electricity (kWhr) for Biocatalyst Production for 1m3 of DW
kWhr_B_Production_m3_DW=Production_Media_Required_L_DW_m3.*EBPT(h); 
...
%for transportation.
kg_B_P_total(h,i,j)=kg_sodium_phosphate_m3_DW+kg_soda_ash_m3_DW+kg_protein_feed_m3_DW...
...
kg_biocatalyst_m3_DW=Volume_Dosing_BO(h)*density_water/L_m3; %The mass of biocatalyst
...
kg_B_ElectronDonor_m3_DW=Total_ElectronDonor_BO_2(h)*mmole_mole*gfm_acetate*g_kg/L_m3; 
%total electron donor (kg) to treat 1m3 of DW

%Total Electron Shuttle Required to treat 1m3 of DW
kg_B_ElectronShuttle_m3_DW=Electron_Shuttle_Dosing_BO(h)/L_m3; 
%conversion is for L to m3 of DW

%Total Electricity (kWhr) for Biocatalyst Treatment Operation for 1m3 of DW
kWhr_B_Operation_m3_DW=(EBOT(h))/L_m3; %conversion is for L to m3 of DW
...
kg_B_Cleaning_m3_DW=V_BC_O_Cleaning_Total(h)*rho_NaClO/Total_Volume_m3_1year; 
%Total NaClO per m3 (normalized over a year with up to 12 cleaning cycles)

%% TRANSPORTATION OF MATERIALS FOR OPERATION

kg_B_O_total(h,i,j)=kg_biocatalyst_m3_DW+kg_B_ElectronDonor_m3_DW+kg_B_ElectronShuttle_m3_DW...
    +kg_B_Cleaning_m3_DW;
...
m3_B_Concrete_m3_DW=V_BC_Concrete_Total_AllTanks(h)/Total_Volume_m3_Facility_Age; 
...
%over 30 years of treatment).
kg_B_Reinf_Steel_m3_DW=(V_BC_Concrete_Total_AllTanks(h)*Reinforce_Steel_m3_Concrete)...
...
%transporation
cost_BP_Transportation_m3_DW(h,i,j)=((kg_B_P_total/1000).*Transportation_Distance_Production(h)).*(price_transportation(h)); %divide cost by 1000 as it is in tons km.
...
CF_BC_P_transportation=((kg_B_P_total/1000).*Transportation_Distance_Production(h))*CF_truck_eff4_32t';
...
CF_BC_O_transportation=((kg_B_O_total/1000).*Transportation_Distance_Operation(h))*CF_truck_eff4_32t';
